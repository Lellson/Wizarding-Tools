...
public class ItemRingOfWaterBreathing extends Item implements IBauble {
	
	public ItemRingOfWaterBreathing() {
	...
	public void onEquipped(ItemStack itemstack, EntityLivingBase player) {
		...
		if (!player.worldObj.isRemote) {
			player.worldObj.playSoundAtEntity(player, "liquid.water", 0.1F, 1.0f);
		}
	}
    ...
    public void addInformation(ItemStack item, EntityPlayer player, List list, boolean b) {
		...
		list.add("Grants regeneration while being in water.");
		}
		if(!GuiScreen.isShiftKeyDown()){
